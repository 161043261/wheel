...
export class TcpPool {
  ...
  async send<T>(data: Buffer | string | object): Promise<T> {
    ...
    const connection = await this.pool.acquire().catch((error) => {
      this.config.logger?.error(
        `Failed to acquire connection: ${getErrorMessage(error)}`,
      );
      throw error;
    });
    try {
      ...
      this.config.logger?.error(
        `Failed to send data: ${getErrorMessage(error)}`,
      );
      throw error;
    } finally {
  ...
  async destroy(): Promise<void> {
    ...
    await this.pool
      ...
      .catch((error) => {
        this.config.logger?.error(
          `Failed to destroy TCP pool: ${getErrorMessage(error)}`,
        );
      });
  ...
  private async createConnection(
    ...
    while (attempt < maxTcpConnectRetries) {
      ...
      connection.onError((error) => {
        this.config.logger?.error(
          `TCP connection error: ${getErrorMessage(error)}`,
        );
      });
      try {
  ...
  private destroyConnection(connection: TcpConnection): void {
    try {
      ...
      this.config.logger?.error(
        `Destroy TCP connection error: ${getErrorMessage(error)}`,
      );
    }
  }
}
